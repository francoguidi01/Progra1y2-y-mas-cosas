...
nodoListaPedidos * agregaProductosListaPedidos(nodoListaPedidos * lista, stProducto p)
{
    nodoListaProducto * prod=inicListaProducto();
    prod=crearNodoListaProducto(p);


    lista->listaDeProductos=agregarAlPrincipioProducto(lista->listaDeProductos, prod);
    return lista;
}
