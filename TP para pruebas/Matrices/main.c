...
int main()
{
    ...
    case 9:
        ...
        fflush(stdin);
        scanf("%s", &palabra);
        flag=encuentraString(3, 3, unaMatrizChar, palabra);
        respuesta(flag);
        break;
    default:
...
//int che(char palabra[],int filas,int columnas,char arregloPalabras[filas][columnas])
//(
//    int value=0;
//    while(i<filas && value ==0)
//        value=strcmp(array,matriz[i]);
//          i++;
//}
int encuentraString (int filas, int columnas, char M[filas][columnas], char palabra[])
{
    int f=0;
    int flag=0;
    printf("[%d]", flag);
    while(f<filas && flag==0)
    {
        if(strcmp(palabra, M[f])==0)
        {
            flag=1;
            printf("[%d]", flag);
        }
...
void respuesta (int flag)
{
    if (flag==1)
    {
        printf("La Enconte gil de goma");
    }
    else
    {
        printf("no");
...
void mostrarMatricesPros(int f, int c, char nombre[f][c], int i,char nombreArchivo[])
{
    int equiposValidos=0;

    equiposValidos=cargarArregloStringEquipos(f,c,nombre,i,nombreArchivo);
    mostrarArregloStrings(f,c,nombre,equiposValidos,i);
}

int cargarArregloStringPro(int f, int j,char nombre[f][j], int i,char nombreArchivo[])
{
    FILE * archivin=NULL;
...
void mostrarArregloStrings(int f,int c, char nombre [f][c],int validos,int i)
{
    int i=0;
    int j=1;
